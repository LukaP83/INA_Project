...
def get_match_passes_by_team(match_passes, team, players, team2, season, team_position):
    # Get passes for team
    ...
    # Take only start 11 players - before first substitution
    substitution = match_passes[match_passes['type'] == 'Substitution']
    substitution_minute = np.min(substitution['minute'])
    match_passes_team = match_passes_team[(match_passes_team['minute'] <= substitution_minute)]
    ...
    if team_position == 0:
        ...
        nodes = pitch.scatter(120 - av_locaction.location_x, av_locaction.location_y,
                                s=350, color = 'blue', edgecolors='black', linewidth=1, alpha = 1, ax = ax)
    ...
    plt.title(f"Pass network for {team} at match againts {team2} - Season {season}")
    plt.show()
...
def get_players_names(match_events, team, team_position):
    # Get player names and jersey numbers
    tactics = match_events[match_events['tactics'].isnull() == False]
    tactics = tactics[['tactics', 'team', 'type']]
    tactics = tactics[tactics['type'] == 'Starting XI']

    # Tactics for one team
    tactics_team = tactics[tactics['team'] == team]
    tactics_team = tactics_team['tactics']

    dict_team = tactics_team[team_position]['lineup']
    lineup_team = pd.DataFrame.from_dict(dict_team)

    # Get only informations about player name and jersey number and save it to dict
    players_team = {}
    for i in range(len(lineup_team)):
        key = lineup_team.player[i]['name']
        val = lineup_team.jersey_number[i]
        players_team[key] = str(val)
